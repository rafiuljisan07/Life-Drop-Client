...
const Aside = () => {
    ...
    return (
        <div>
            ...
            <div className={`fixed inset-y-0 left-0 w-64 bg-red-900 text-white p-6 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'
                } md:translate-x-0 md:min-h-screen z-40`}>
                {isOpen && (
                    <button
                        onClick={() => setIsOpen(false)}
                        className="md:hidden fixed top-2 right-2 z-50 p-2 text-xl"
                    >
                        ✕
                    </button>
                )}
                <h2 className="text-2xl font-bold mb-8">Life Drop</h2>
